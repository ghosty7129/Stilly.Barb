...
class EmailService {
  ...
  async sendConfirmationEmail({
    ...
    const mailOptions = {
      ...
      html: `<!doctype html><html><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width,initial-scale=1" /></head><body style="margin:0;padding:0;background:#f3f4f6;font-family:Segoe UI, Helvetica, Arial, sans-serif;color:#0f172a;"><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="background:#f3f4f6;width:100%;"><tr><td align="center" style="padding:10px 8px;"><table role="presentation" cellpadding="0" cellspacing="0" border="0" width="720" style="width:100%;max-width:720px;background:#ffffff;border:1px solid #e6e9ee;border-radius:10px;overflow:hidden;"><tr><td style="background:#8a8f99;padding:20px 16px 16px;text-align:center;color:#ffffff;"><div style="width:48px;height:48px;border-radius:999px;background:#ffffff;color:#7b8794;display:inline-block;line-height:48px;font-size:24px;font-weight:700;margin-bottom:8px">✓</div><h1 style="margin:0;font-size:18px;line-height:1.3;font-weight:700;color:#ffffff;letter-spacing:0.1px;word-break:break-word;white-space:normal;overflow-wrap:anywhere">${escapeHtml(labels.title)}</h1><p style="margin:6px 0 0;font-size:12px;line-height:1.35;color:rgba(255,255,255,0.92)">${escapeHtml(labels.subtitle)}</p></td></tr><tr><td style="padding:12px 12px 14px;"><h2 style="margin:0 0 8px;font-size:15px;line-height:1.2;font-weight:800;color:#0f172a">${escapeHtml(labels.details)}</h2><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;background:#fbfbfc;border:1px solid #eef2f6;border-radius:10px;"><tr><td style="padding:10px"><div style="font-size:10px;line-height:1.15;color:#6b7280;margin-bottom:2px">${escapeHtml(labels.fullName)}</div><div style="font-size:15px;line-height:1.3;font-weight:700;color:#0f172a;white-space:normal;overflow-wrap:anywhere">${safeName}</div></td></tr></table><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;background:#fbfbfc;border:1px solid #eef2f6;border-radius:10px;"><tr><td style="padding:10px"><div style="font-size:10px;line-height:1.15;color:#6b7280;margin-bottom:2px">${escapeHtml(labels.email)}</div><div style="font-size:14px;line-height:1.3;font-weight:700;color:#0f172a;word-break:break-word;white-space:normal;overflow-wrap:anywhere">${safeEmail}</div></td></tr></table><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;background:#fbfbfc;border:1px solid #eef2f6;border-radius:10px;"><tr><td style="padding:10px"><div style="font-size:10px;line-height:1.15;color:#6b7280;margin-bottom:2px">${escapeHtml(labels.phone)}</div><div style="font-size:15px;line-height:1.3;font-weight:700;color:#0f172a">${escapeHtml(phone)}</div></td></tr></table><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;background:#fbfbfc;border:1px solid #eef2f6;border-radius:10px;"><tr><td style="padding:10px"><div style="font-size:10px;line-height:1.15;color:#6b7280;margin-bottom:2px">${escapeHtml(labels.service)}</div><div style="font-size:16px;line-height:1.15;font-weight:800;color:#0f172a;white-space:normal;overflow-wrap:anywhere">${safeService}</div><div style="font-size:12px;line-height:1.25;color:#475569;margin-top:4px">${resolvedServiceDuration ? `${resolvedServiceDuration} ${language === 'en' ? 'minutes' : 'мин'}` : ''}${resolvedServiceDuration ? ' • ' : ''}${formatMoney(resolvedServicePrice)}</div></td></tr></table>${normalizedAddons.length > 0 ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;background:#fbfbfc;border:1px solid #eef2f6;border-radius:10px;"><tr><td style="padding:10px"><div style="font-size:10px;line-height:1.15;color:#6b7280;margin-bottom:4px">${escapeHtml(labels.addons)}</div>${normalizedAddons.map(addon => `<div style="font-size:13px;line-height:1.2;font-weight:600;color:#0f172a;margin-bottom:2px;white-space:normal;overflow-wrap:anywhere">${escapeHtml(addon.name)}</div><div style="font-size:11px;line-height:1.2;color:#6b7280;margin-bottom:4px">${addon.duration ? `${addon.duration} ${language === 'en' ? 'min' : 'мин'}` : ''} ${addon.duration && addon.price ? '• ' : ''}${addon.price ? formatMoney(addon.price) : ''}</div>`).join('')}</td></tr></table>` : ''}<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;background:#f3f4f6;border:1px solid #4b5563;border-radius:10px;"><tr><td style="padding:10px"><div style="font-size:10px;line-height:1.15;color:#6b7280;margin-bottom:2px">${escapeHtml(labels.dateTime)}</div><div style="font-size:15px;line-height:1.3;font-weight:700;color:#0f172a">${escapeHtml(formattedDate)}</div><div style="font-size:12px;line-height:1.25;color:#475569;margin-top:2px">${formattedTime}</div></td></tr></table><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-bottom:8px;"><tr><td style="padding:0"><div style="display:block;border-radius:8px;border:1px solid #e6e9ee;background:#f8fafc;padding:10px 12px;font-weight:800;"><span style="font-size:13px;color:#0f172a">${escapeHtml(labels.totalPrice)}:&nbsp;</span><span style="float:right;font-size:16px;color:#7c3aed">${formatMoney(resolvedTotalPrice)}</span><div style="clear:both"></div></div></td></tr></table>${resolvedTotalDuration ? `<div style="margin-bottom:8px;color:#6b7280;font-size:11px">${language === 'en' ? 'Total duration' : 'Общо време'}: ${totalDurationLabel}</div>` : ''}<div style="padding:10px;background:#f1f5f9;border:1px solid #e2e8f0;border-radius:8px;font-size:12px;line-height:1.3;color:#0f172a;"><strong>${escapeHtml(labels.confirmationNote)}</strong> ${escapeHtml(labels.confirmationEmailNote(email))}</div></td></tr></table></td></tr></table></body></html>`
    };
